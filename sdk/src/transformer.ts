...
export interface TransformerConfig<TIn, TOut> extends CellConfig {
  process: (input: TIn) => Promise<TOut>;
  children?: UniversalContract[];
  complexityBudget?: number;
}

export class Transformer<TIn = unknown, TOut = unknown> implements UniversalContract {
  ...
  private readonly _process: (input: TIn) => Promise<TOut>;
  ...
  constructor(config: TransformerConfig<TIn, TOut>) {
    this.identity = { name: config.name, cellType: "transformer", version: config.version };
    ...
    // Placeholder signature for computation (hash is overwritten immediately)
    this.signature = { hash: "0".repeat(64), timestamp: new Date().toISOString() };
    this.signature = this.computeSignature();
  }

  /** Execute the transformer. Decrements complexity budget; enters Safe-Mode at zero. */
  async process(input: TIn): Promise<TOut> {
    if (this._complexityBudget <= 0) {
      throw new Error(`[Circuit Breaker] Transformer "${this.identity.name}" is in Safe-Mode — complexity budget exhausted`);
    }
